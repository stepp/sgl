...
void GWindow::setMenuItemEnabled(const std::string& menu, const std::string& item, bool enabled) {
    ...
    } else if (_menuActionMap.find(menuItemKey) == _menuActionMap.end()) {
        throw std::runtime_error("GWindow::setMenuItemEnabled: menu item \"" + item + "\" does not exist");
    }
...
bool _Internal_QMainWindow::timerExists(int id) {
    if (id >= 0) {
        return _timerIDs.find(id) != _timerIDs.end();
    } else {
        return !_timerIDs.empty();
    }
}

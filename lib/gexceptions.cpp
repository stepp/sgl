...
bool getTopLevelExceptionHandlerEnabled() {
    ...
    return std::get_terminate() == sglTerminateHandler;
#else
    return true;
#endif
}
...
void setTopLevelExceptionHandlerEnabled(bool enabled) {
    ...
    if (enabled) {
        void (* previous)() = std::set_terminate(sglTerminateHandler);
        if (previous != sglTerminateHandler) {
...
#if _WIN32
    for (int sig : STATIC_VARIABLE(SIGNALS_HANDLED)) {
        signal(sig, sglSignalHandler); // Windows only signal, not sigaction, sigh
    }
#else
    struct sigaction action;
    action.sa_handler = sglSignalHandler;
    action.sa_flags = 0;
...
[[noreturn]] static void sglTerminateHandler() {
    ...
    try {
        ...
        event = "A C++ runtime error was thrown during program execution";
        details = ex.what();

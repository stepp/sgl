...
void GObject::setSize(double width, double height) {
    if (isTransformed()) {
        throw std::runtime_error("GObject::setSize: Object has been transformed");
    }
    _width = width;
    _height = height;
    repaint();
...
bool GArc::contains(double x, double y) const {
    if (isTransformed()) {
        // TODO
        // return sgl::getPlatform()->gobject_contains(this, x, y);
    }
...
GRectangle GArc::getBounds() const {
    if (isTransformed()) {
        // TODO
        // return sgl::getPlatform()->gobject_getBounds(this);
    }
    double rx = getWidth() / 2;
    double ry = getHeight() / 2;
    double cx = getX() + rx;
...
GRectangle GCompound::getBounds() const {
    ...
    for (int i = 0; i < _contents.size(); i++) {
        GRectangle bounds = _contents[i]->getBounds();
...
void GCompound::removeAll() {
    bool wasEmpty = _contents.empty();
    std::vector<GObject*> contentsCopy = _contents;
    _contents.clear();
...
void GCompound::sendBackward(GObject* gobj) {
    ...
    if (index != 0) {
        _contents.erase(_contents.begin() + index);
        _contents.insert(_contents.begin() + index - 1, gobj);
        // sgl::getPlatform()->gobject_sendBackward(gobj);
        conditionalRepaint();
    }
}
...
void GCompound::sendForward(GObject* gobj) {
    ...
    if (index != _contents.size() - 1) {
        _contents.erase(_contents.begin() + index);
        _contents.insert(_contents.begin() + index + 1, gobj);
        // sgl::getPlatform()->gobject_sendForward(gobj);
        conditionalRepaint();
    }
}
...
void GCompound::sendToBack(GObject* gobj) {
    ...
    if (index != 0) {
        _contents.erase(_contents.begin() + index);
        _contents.insert(_contents.begin(), gobj);
        // sgl::getPlatform()->gobject_sendToBack(gobj);
        conditionalRepaint();
    }
}
...
GImage::GImage(const std::string& filename, double x, double y)
        : GObject(x, y),
          ...
          _qimage(nullptr) {
    if (!load(filename)) {
        throw std::runtime_error("GImage::constructor: unable to load image from: \"" + filename + "\"");
    }
}
...
GImage::GImage(std::istream& source, double x, double y)
    ...
    if (!loadFromStream(source)) {
        throw std::runtime_error("GImage::constructor: unable to load image from stream input");
    }
}
...
std::vector<GPoint> GPolygon::getVertices() const {
    std::vector<GPoint> vec;
    for (const QPointF& point : _vertices) {
        vec.push_back(GPoint(point.x(), point.y()));
    }
    return vec;
}

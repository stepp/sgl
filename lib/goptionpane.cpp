...
GOptionPane::ConfirmResult GOptionPane::showConfirmDialog(QWidget* parent,
    ...
    if (type != GOptionPane::ConfirmType::CONFIRM_YES_NO
        ...
        throw std::runtime_error("GOptionPane::showConfirmDialog: Illegal dialog type");
    }
...
void GOptionPane::showMessageDialog(QWidget* parent,
    ...
    if (type != GOptionPane::MessageType::MESSAGE_PLAIN
        ...
        throw std::runtime_error("GOptionPane::showMessageDialog: Illegal dialog type");
    }
...
std::string GOptionPane::showOptionDialog(QWidget* parent,
    ...
    GThread::runOnQtGuiThread([parent, message, &options, titleToUse, initiallySelected, &result]() {
        ...
        std::set<QAbstractButton*> buttonsUsed;
        std::set<std::string> charsUsed;
        ...
        for (int i = 0; i <= 2; i++) {
            ...
            for (QAbstractButton* button : box.buttons()) {
                if (buttonsUsed.find(button) != buttonsUsed.end()) {
                    buttonIndex++;
                    continue;
                }
                ...
                if (charsUsed.find(letter) != charsUsed.end()) {
                    buttonIndex++;
                    continue;
                }

                buttonsUsed.insert(button);
                charsUsed.insert(letter);

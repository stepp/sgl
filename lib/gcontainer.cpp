...
void GContainer::addToRegion(GInteractor* interactor, Region region) {
    ...
    interactor->setContainer(this);
    _interactors.push_back(interactor);
    _interactorsByRegion[region].push_back(interactor);
...
void GContainer::clearRegion(Region region) {
    for (GInteractor* interactor : _interactorsByRegion[region]) {
        ...
        std::remove(_interactors.begin(), _interactors.end(), interactor);
    }
    _interactorsByRegion.erase(region);
...
std::vector<GInteractor*> GContainer::getDescendents(const std::string& type) const {
...
const std::vector<GInteractor*>& GContainer::getInteractors() const {
    return _interactors;
}
...
GInteractor* GContainer::getInteractorByRegion(int i, Region region) const {
    return _interactorsByRegion.at(region).at(i);
}
...
int GContainer::getInteractorCountByRegion(Region region) const {
    return _interactorsByRegion.at(region).size();
}
...
void GContainer::insert(int index, GInteractor* interactor) {
    ...
    interactor->setContainer(this);
    _interactors.insert(_interactors.begin() + index, interactor);
...
void GContainer::insertToRegion(int index, GInteractor* interactor, Region region) {
    ...
    interactor->setContainer(this);
    _interactors.push_back(interactor);
    _interactorsByRegion[region].insert(_interactorsByRegion[region].begin() + index, interactor);
...
void GContainer::remove(int index) {
    ...
    interactor->setContainer(nullptr);
    _interactors.erase(_interactors.begin() + index);
...
void GContainer::removeFromRegion(GInteractor* interactor, Region region) {
    ...
    interactor->setContainer(nullptr);
    std::remove(_interactors.begin(), _interactors.end(), interactor);
    std::remove(_interactorsByRegion[region].begin(), _interactorsByRegion[region].end(), interactor);
...
void GContainer::removeFromRegion(int index, Region region) {
    ...
    interactor->setContainer(nullptr);
    std::remove(_interactors.begin(), _interactors.end(), interactor);
    _interactorsByRegion[region].erase(_interactorsByRegion[region].begin() + index);
...
void _Internal_QContainer::add(QWidget* widget) {
    switch (_layoutType) {
        ...
        default: {
            throw std::runtime_error("GContainer::add: no layout has been set");
            break;
        }
    }
}
...
void _Internal_QContainer::fixAlignment(QWidget* widget, GContainer::Region region) {
    ...
    if (_layoutType == GContainer::LAYOUT_BORDER) {
        QLayout* layout = layoutForRegion(region);
        if (_halignMap.find(region) != _halignMap.end()
                && _valignMap.find(region) != _valignMap.end()) {
            layout->setAlignment(widget, toQtAlignment(_halignMap[region]) | toQtAlignment(_valignMap[region]));
        } else if (_halignMap.find(region) != _halignMap.end()) {
            layout->setAlignment(widget, toQtAlignment(_halignMap[region]));
        } else if (_valignMap.find(region) != _valignMap.end()) {
...
bool _Internal_QContainer::isRegionStretch(GContainer::Region region) const {
    return _regionStretchMap.find(region) != _regionStretchMap.end() ? _regionStretchMap.at(region) : false;
}

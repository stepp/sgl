...
std::map<QThread*, GThread*> GThread::_allGThreadsQt;
std::map<std::thread*, GThread*> GThread::_allGThreadsStd;
...
/*static*/ void GThread::ensureThatThisIsTheQtGuiThread(const std::string& message) {
    if (!iAmRunningOnTheQtGuiThread()) {
        throw std::runtime_error((message.empty() ? "" : (message + ": "))
...
/*static*/ GThread* GThread::getCurrentThread() {
    ...
    if (_allGThreadsQt.find(currentQtThread) != _allGThreadsQt.end()) {
...
/*static*/ void GThread::runOnQtGuiThread(GThunk func) {
    if (iAmRunningOnTheQtGuiThread()) {
        ...
        throw std::runtime_error("GThread::runOnQtGuiThread: Qt GUI thread has not been initialized properly. \n"
            "Make sure that the file containing your main() function #includes at least \n"
            "one .h header from the SGL C++ library.");
    }
}
...
/*static*/ void GThread::runOnQtGuiThreadAsync(GThunk func) {
    if (iAmRunningOnTheQtGuiThread()) {
        ...
        throw std::runtime_error("GThread::runOnQtGuiThreadAsync: Qt GUI thread has not been initialized properly. \n"
            "Make sure that the file containing your main() function #includes at least \n"
            "one .h header from the SGL C++ library.");
    }
}
...
/*static*/ bool GThread::wait(GThread* thread, long ms) {
    ...
    if (currentThread == thread) {
        throw std::runtime_error("GThread::wait: a thread cannot wait for itself");
    }
...
bool GThreadQt::join(unsigned long ms) {
...
GThreadStd::~GThreadStd() {
    ...
    _allGThreadsStd.erase(_stdThread);
    _running = false;
    _stdThread = nullptr;
}
...
// I think the current behavior tells this thread to pause itself,
// while the desired behavior is to have the calling thread wait for this thread.
bool GThreadStd::join(unsigned long ms) {
    long elapsed = 0;

...
/*static*/ const std::map<std::string, int>& GColor::colorTable() {
...
/*static*/ const std::map<std::string, std::string>& GColor::colorNameTable() {
    if (_colorNameTable.empty()) {
...
/*static*/ std::string GColor::convertARGBToColor(int a, int r, int g, int b) {
    if (a < 0 || a > 255 || r < 0 || r > 255 || g < 0 || g > 255 || b < 0 || b > 255) {
        throw std::runtime_error("GColor::convertARGBToColor: invalid ARGB value (must be 0-255)");
    }
...
/*static*/ int GColor::convertColorToRGB(const std::string& colorName) {
    ...
    if (colorName[0] == '#') {
        ...
        if (terminator != '@') {
            throw std::runtime_error("GColor::convertColorToRGB: Illegal color - \"" + colorName + "\"");
        }
        return static_cast<int>(rgb & 0xffffffff);
    }
    std::string name = canonicalColorName(colorName);
    colorTable();
    if (_colorTable.find(name) == _colorTable.end()) {
        throw std::runtime_error("GColor::convertColorToRGB: Undefined color - \"" + colorName + "\"");
    }
...
/*static*/ std::string GColor::convertRGBToColor(int r, int g, int b) {
    if (r < 0 || r > 255 || g < 0 || g > 255 || b < 0 || b > 255) {
        throw std::runtime_error("GColor::convertRGBToColor: invalid RGB value (must be 0-255)");
    }
    std::ostringstream os;
    os << std::hex << std::setfill('0') << std::uppercase << "#";
    os << std::setw(2) << (r & 0xFF);

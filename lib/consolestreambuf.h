 ...
 * represents a stream buffer that reads/writes to the graphical console
 * using a process pipe to a Java back-end process.
 *
 * @version 2021/04/03
 * - removed dependency on error function
 * @version 2016/10/04
 * - initial version
 */
...
public:
    ...
    virtual int underflow() {
        ...
        int n = line.length();
        if (n + 1 >= BUFFER_SIZE) {
            throw std::runtime_error("ConsoleStreambuf::underflow: String too long");
        }
        for (int i = 0; i < n; i++) {
            inBuffer[i] = line[i];
        }
 ...
 */
class ConsoleStreambufQt : public ::sgl::ConsoleStreambuf {
public:

...
public:
     ...
     */
    virtual std::vector<GInteractor*> getDescendents(const std::string& type = "") const;
...
private:
    ...
    std::vector<GInteractor*> _interactors;
    std::map<Region, std::vector<GInteractor*>> _interactorsByRegion;
    Layout _layout;
...
private:
    ...
    std::map<GContainer::Region, HorizontalAlignment> _halignMap;
    std::map<GContainer::Region, VerticalAlignment> _valignMap;
    std::map<GContainer::Region, bool> _regionStretchMap;

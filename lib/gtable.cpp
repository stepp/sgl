...
GTable::GTable(int rows, int columns, double width, double height, QWidget* parent)
    ...
    if (rows < 0 || columns < 0 || width < 0 || height < 0) {
        throw std::runtime_error("GTable::constructor: invalid negative parameters");
    }
...
void GTable::checkColumn(const std::string& member, int column) const {
    if (column < 0 || column > numCols()) {
        throw std::runtime_error("GTable::" + member + ": column value out of range");
    }
}

void GTable::checkIndex(const std::string& member, int row, int column) const {
    if (row < 0 || column < 0 || row > numRows() || column > numCols()) {
        throw std::runtime_error("GTable::" + member + ": row or column value out of range");
    }
}

void GTable::checkRow(const std::string& member, int row) const {
    if (row < 0 || row > numRows()) {
        throw std::runtime_error("GTable::" + member + ": row value out of range");
    }
}
...
void GTable::ensureColumnStyle(int column) {
    ensureDefaultFormatting();
    if (_columnStyles.find(column) == _columnStyles.end()) {
...
void GTable::ensureRowStyle(int row) {
    ensureDefaultFormatting();
    if (_rowStyles.find(row) == _rowStyles.end()) {
...
void GTable::resize(int newNumRows, int newNumCols) {
    if (newNumRows < 0 || newNumCols < 0) {
        throw std::runtime_error("GTable::resize: invalid negative rows or columns value");
    }
...
void GTable::setBackground(int rgb) {
    ...
    GThread::runOnQtGuiThread([this, rgb]() {
        ...
        for (auto it = _rowStyles.begin(); it != _rowStyles.end(); it++) {
            it->second.background = unset.background;
        }
        for (auto it = _columnStyles.begin(); it != _columnStyles.end(); it++) {
            it->second.background = unset.background;
        }
...
void GTable::setColumnWidth(int column, double width) {
    ...
    if (width < 0) {
        throw std::runtime_error("GTable::setColumnWidth: width cannot be negative");
    }
...
void GTable::setForeground(int rgb) {
    GThread::runOnQtGuiThread([this, rgb]() {
        ...
        TableStyle unset = TableStyle::unset();
        for (auto it = _rowStyles.begin(); it != _rowStyles.end(); it++) {
            it->second.foreground = unset.foreground;
        }
        for (auto it = _columnStyles.begin(); it != _columnStyles.end(); it++) {
            it->second.foreground = unset.foreground;
        }
...
void GTable::setHorizontalAlignment(HorizontalAlignment alignment) {
    GThread::runOnQtGuiThread([this, alignment]() {
        ...
        TableStyle unset = TableStyle::unset();
        for (auto it = _rowStyles.begin(); it != _rowStyles.end(); it++) {
            it->second.alignment = unset.alignment;
        }
        for (auto it = _columnStyles.begin(); it != _columnStyles.end(); it++) {
            it->second.alignment = unset.alignment;
        }
...
void GTable::setRowHeight(int row, double height) {
    ...
    if (height < 0) {
        throw std::runtime_error("GTable::setRowHeight: height cannot be negative");
    }
...
_Internal_QTableWidget::_Internal_QTableWidget(GTable* gtable, int rows, int columns, QWidget* parent)
        : QTableWidget(rows, columns, parent),
          ...
          _delegate(nullptr) {
    if (!gtable) {
        throw std::runtime_error("_Internal_QTableWidget::constructor: null table");
    }
...
void _Internal_QTableWidget::keyPressEvent(QKeyEvent* event) {
    if (!event) {
        throw std::runtime_error("_Internal_QTableWidget::keyPressEvent: null event");
    }
    if (!_gtable) {

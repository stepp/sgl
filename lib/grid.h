...
private:
    /* Instance variables */
    std::vector<ValueType> _elements;   // The elements, in row-major order
...
Grid<ValueType>::Grid(std::initializer_list<std::initializer_list<ValueType>> list) {
    ...
    for (int row = 0; row < _rowCount; row++) {
        if (static_cast<int>(rowItr->size()) != _columnCount) {
            throw std::runtime_error("Grid::constructor: initializer list is not rectangular (must have same # cols in each row)");
        }
...
void Grid<ValueType>::resize(int numRows, int numCols, bool retain) {
    ...
    std::vector<ValueType> oldElements = std::move(_elements);
    ...
    this->_elements = std::vector<ValueType>(numRows * numCols, ValueType());
...
const T& randomElement(const Grid<T>& grid) {
    if (grid.isEmpty()) {
        throw std::runtime_error("randomElement: empty grid was passed");
    }

...
std::string integerToString(int n, int radix) {
    if (radix <= 0) {
        throw std::runtime_error("integerToString: Illegal radix: " + std::to_string(radix));
    }
...
std::string longToString(long n, int radix) {
    if (radix <= 0) {
        throw std::runtime_error("longToString: Illegal radix: " + std::to_string(radix));
    }
    std::ostringstream stream;
    if (radix != 10) {
        stream << std::setbase(radix);
...
bool stringIsInteger(const std::string& str, int radix) {
    if (radix <= 0) {
        throw std::runtime_error("stringIsInteger: Illegal radix: " + std::to_string(radix));
    }
...
bool stringIsLong(const std::string& str, int radix) {
    if (radix <= 0) {
        throw std::runtime_error("stringIsLong: Illegal radix: " + std::to_string(radix));
    }
    std::istringstream stream(trim(str));
    stream >> std::setbase(radix);
    long value;
...
bool stringToBool(const std::string& str) {
    ...
    if (stream.fail() || !stream.eof()) {
        throw std::runtime_error("stringToBool: Illegal bool format (" + str + ")");
    }
    return value;
}
...
char stringToChar(const std::string& str) {
    ...
    if ((int) str2.length() != 1) {
        throw std::runtime_error("stringToChar: string must contain exactly 1 non-whitespace character");
    }
    return str2[0];
}
...
int stringToInteger(const std::string& str, int radix) {
    if (radix <= 0) {
        throw std::runtime_error("stringToInteger: Illegal radix: " + std::to_string(radix));
    }
    std::istringstream stream(trim(str));
    stream >> std::setbase(radix);
    int value;
    stream >> value;
    if (stream.fail() || !stream.eof()) {
        throw std::runtime_error("stringToInteger: Illegal integer format: \"" + str + "\"");
    }
    return value;
}
...
long stringToLong(const std::string& str, int radix) {
    if (radix <= 0) {
        throw std::runtime_error("stringToLong: Illegal radix: " + std::to_string(radix));
    }
    std::istringstream stream(trim(str));
    stream >> std::setbase(radix);
    long value;
    stream >> value;
    if (stream.fail() || !stream.eof()) {
        throw std::runtime_error("stringToLong: Illegal long format \"" + str + "\"");
    }
    return value;
}
...
double stringToReal(const std::string& str) {
    ...
    stream >> value;
    if (stream.fail() || !stream.eof()) {
        throw std::runtime_error("stringToReal: Illegal floating-point format (" + str + ")");
    }
    return value;
}
    ...
    for (std::string::const_iterator i = str.begin(), n = str.end(); i != n; ++i) {
        ...
        if (isalnum(c) || c == '-' || c == '_' || c == '.' || c == '~' || c == '*') {
            ...
            if (i + 2 >= n || !isxdigit(*(i + 1)) || !isxdigit(*(i + 2))) {
                throw std::runtime_error("urlDecode: Invalid percent-encoding");
            }

...
void GDownloader::downloadInternal() {
    GThread::runOnQtGuiThreadAsync([this]() {
        ...
        for (auto it = _headers.begin(); it != _headers.end(); it++) {
            request->setRawHeader(QByteArray(it->first.c_str()), QByteArray(it->second.c_str()));
        }
        ...
        _reply->connect(
                _reply, SIGNAL(std::runtime_error(QNetworkReply::NetworkError)),
...
void GDownloader::fileDownloadError(QNetworkReply::NetworkError nerror) {
    throw std::runtime_error("file download error: " + std::to_string(nerror));
}
    ...
    if (_reply) {
        ...
        if (nerror) {
            ...
            if (!outfile.open(QIODevice::WriteOnly)) {
                throw std::runtime_error("GDownloader::" + member + ": cannot open file " + filename + " for writing");
            }
